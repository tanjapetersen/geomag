	program hour1s
c   This program reads 1 sec fluxgate .txt files and 5 sec proton .txt files
c   Also reads the seperate Benmore file, to combine with other files
...
!   NEW CORRECTION IF NO benmore file
!   NOW TRYING ALTERNATIVE BENMORE CORRECTION
!
...
!  Expected bias values are put in here also
	if(stn .eq.'eyr') then
	   ...
	   fileg = 'data/'//yearday//'.'//hrstr//'00.00.gsm-eyrewell.txt'
...
!	open(20,file= stc//'/' // fileo,access='append')
	open(20,file= stc//'/' // fileo )      !  NEW VERSION
...
!	print *, e0,' ',e1,' ',e2,' ',e3,' ',e4,' ',zoffset
 ...
 3000	format(2i5,1x,2f7.2,3f9.4,f11.3,f10.3,f11.3,f9.4)
	bsum = 0.0
	bsumsq = 0.0
	do i = 0,3599		! Fill array with null readings
	   ...
	   ddata(i,2) = 0.
	   ddata(i,9) = 0.
           if ( stn .eq. 'eyr' ) ddata(i,9) = -1000.
	   ddata(i,11) = 0.
	   ddata(i,12) = 0.
	end do    ! for i

!  If Eyrewell, read Benmore Basalt Correction File
        if ( stn .eq. 'eyr' ) then
 1200	format(27x,3(i2,1x),4x,f9.4)
	   do i = 0,3599		! For 1 hour of 1 sec readings
	      read(12,1200,end=160)bh,bm,bs,ben
!  Now adjust Benmore to match old system
!	      ben = ben * 1/0.073
!	      write(41,*) bh,bm,bs, ben
  ...
  160	      continue
	   ...
	   if(ddata(0,9) .lt. -999.0) ddata(0,9)= 0.0	! avoid complete lockup
	   do i = 1,3599	! Normally assume will always be first reading
	      write(40,*) i,ddata(i,9), ddata(i-1,9)
	      if(ddata(i,9) .lt. -999.0) ddata(i,9)=ddata(i-1,9)
	   end do
        end if
  ...
  111	   j = im*60+is	
     ...
     &        "Problem ",ihr,ih,im,is, i, j
 	   write(37,3000) i,j,st,dt,xr,yr,zr,xc,yc,zc,ddata(j,9)
	   if(abs(xc) .gt. 1) then
	      ...
	      ddata(j,4) = yc
	      ddata(j,5) = zc
!  Next line should only occur if no Benmore file
	      if(ddata(j,9) .lt. -999.0) ddata(j,9) = 0.0
...
c	   write(*,*) linef
 ...
 1100	format(3i3,f9.2,2i3,f6.1)
	vav = 0.0
	do i = 0,719		! assumes no extra readings, could change
	   read(11,1100,end=200) ih,im,is,f,g,q,v
...
!   Constant of .931 based on effect of Z on F for 2006 Average values
	      ddata(j*5,10) = f - 0.931 * bfact * ddata(j*5,9) + fcor
	      ddata(j*5,11) = g
	      ddata(j*5,12) = q
	      vav = vav + v
!  Next lines compare Fs with Fcalc
!
	      if(ddata(j*5,10) .gt. 50000.) then
	         fcalc = sqrt(ddata(j*5,6)*ddata(j*5,6)+ddata(j*5,8)*
     &                         ddata(j*5,8)) 
!	         if(abs(ddata(j*5,10)-fcalc) .gt. ftol) write(21,*) 
!    &                   ih,im,is,ddata(j*5,10)-fcalc,g,q," Wrong Total"

	         if ((ddata(j*5,11).lt.1.1).or.(ddata(j*5,12) .lt. 5.1)) 
     &           write(21,*) ih,im,is,ddata(j*5,11),ddata(j*5,12),
     &           " Poor Proton"
	         if((abs(ddata(j*5,10)-fcalc) .gt. 2.0) .and.
     &             ((ddata(j*5,11).lt.3.1).or.(ddata(j*5,12) .lt. 5.1)))
     &           ddata(j*5,10) = 0.0
	      else
...
!   For GSM, only 0,5,10 etc second readings exist
 	   do i = 0,719
c	      write(21,*) "XXX ",ihr,i,j,ddata(i,j)
 	      call gsm_med(i,med)
 	      if (abs(ddata(i*5,10) - med) .gt. 50) then
 	         im = i/12
		 is = 5 * i - 12*im
