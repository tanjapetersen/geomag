...
!   the exact 15 minutes 
	...
	open(14,file= "iono_pro.dat")		! Corrections input file
...
!
	do i = 0,30
	   read(14,*) idum, S(1,i), S(2,i), S(3,i)	! idum to be 0..30
	end do				 
...
!             lines 0:3599 are current hour
	...
	do i = 0,3599		! assumes no extra readings
     ...
     &     data(i,3),data(i,4), data(i,5),data(i,6),data(i,7)
	   k = i - (i/900)*900	
 	   if((k .ge. idel).and.(k .le. (idel+30))) then
	      do j = 1,3
	         data(i,j) = data(i,j) - S(j,k-idel)
	      end do
	   end if
