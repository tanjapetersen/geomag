...
!  If Eyrewell, read Benmore Correction File
        if ( stn .eq. 'eyr' ) then
!  Read benmore file header
	   linef(1:2) = '  ' 
	   do while (linef(1:2) .ne. 'HH')
 	      read(12, '(a10)',end=150) linef	! OK to process with no gsm file
!	      write(*,*) 'Benmore  ',linef
	   end do
!	   do i = 1,25
!	      read(12, '(a10)',end=150) linef	! OK to finish early
!	   end do
  150	   continue
	   do i = 0,3599		! For 1 hour of 1 sec readings
	      read(12,1200,end=160)bh,bm,bs,ben
!	      write(40,*) bh,bm,bs, ben
	      if(abs(ben) .gt. 9.9) ben = 0.0	!  
	      j = bm*60+bs	
  	      if((j .eq. 0).and.(i .gt. 0)) goto 160		! in case blank line at end
 	      ddata(bm*60+bs,9) = ben*100
  160	      continue
	   end do
	   do i = 1,3599		! Assume will always be first reading
