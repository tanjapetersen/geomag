...
! NOTE: If you have a lot of missing days (all 99999.00 values) you might want to run it in chunks (e.g set up a test are with only January 2016 data in your list.eyr and in eyr/ subdirectory).
	...
	character*10 fileday
        character*30 cd1, cd2, cd3
 	character*72 line,lineh
	common /a/ data	
...
!  Set up files and parameters for stations here
	   ...
	   open(11,file = 'done/eyr20'//fileday(1:6)//'pmin.min')
!  Read header, so ready to start reading data
           header = 1
           do while (header .ne. 0)
              read(11,'(a70)') lineh
              if(lineh(:1) .ne. ' ') header = 0
           end do
 ...
 1000	format(11x,i2,x,i2,14x,4f10.2)	   
  140	   do i = 0,1439	! now 1 minute readings, ixyz is dummy for day
     ...
     &            data(i,3),data(i,4),data(i,5),data(i,6),data(i,7)
              read(11,*,end=200) cd1, cd2, cd3,data(i,1),data(i,2),
     &                           data(i,3),data(i,4)
