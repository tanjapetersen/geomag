	program gdasout
c
c  Program to convert the IAGA2002 SBA files from GDASView 
...
c
c  It reads a gdasview output pmin file (HDZF), e.g. sba20150101pmin.min, in the sba sub-c  directory and 
c  writes an XYZF pmin file with the header from sba_header.txt in sub-directory new.
c
c  NOTE:  While gdasout.f is good for API & EYR, this version of the program accounts for GDASView 
c  producing wrong Declination outputs (at SBA you are looking North to the South Pole so D is about 150 degrees)
c
c  compiles with f77
...
! SBA needs a minus in front of H to correct the incorrect declination produced by GDASView:
                          x = -h * cos(rad)
                          y = -h * sin(rad)
		       else
			  x = 99999.
