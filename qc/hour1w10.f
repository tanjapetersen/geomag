...
c             A spike is defined by a data point being >10 above the median calculated for 12-sec chunks of data (in hour1w.f the threshold is 500 instead of 10)
 	      if (abs(ddata(i,j) - med) .gt. 10) then
               im = i/60
		 is = i - 60*im
