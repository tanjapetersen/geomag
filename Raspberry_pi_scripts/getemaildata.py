...
resp, items = mail.search(None, '(UNSEEN FROM "tanjap@toho.gns.cri.nz")')
if len(items) > 0:
    try:
        items = items[0].split()
        ...
        for part in msg.walk():
            ...
            hour_now = datetime.datetime.utcnow().hour
            ...
            keepif = datetime.timedelta(days=4) # < 3 days ago
            if datetime.datetime.strptime(re.findall(r'\d+',
                ...
                np.save(os.path.join('/home/pi/Data', newfilename),
                        magdata)
